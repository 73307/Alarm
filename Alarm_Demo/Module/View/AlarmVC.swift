...
class AlarmVC: UITableViewController{
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        sw.addTarget(self, action: #selector(AlarmVC.switchTapped(_:)), for: UIControl.Event.valueChanged)
    ...
    @objc func handleChangeNotification(_ notification: Notification) {
        ...
        if let changeReason = userInfo[Alarm.changeReasonKey] as? String {
            ...
            case let (Alarm.added, (index as Int)?, _):
                ...
                scheduler.setNotification(alarmLabel: alarm.label ,date: alarm.date, ringtoneName: alarm.mediaLabel, repeatWeekdays: alarm.repeatWeekdays, snoozeEnabled: alarm.snoozeEnabled, onSnooze: false, uuid: alarm.uuid.uuidString)
            case let (Alarm.updated, (index as Int)?, _):
                let alarm = alarms[index]
                let uuid = alarm.uuid.uuidString
                if alarm.enabled {
                    scheduler.updateNotification(alarmLabel: alarm.label,ByUUIDStr: uuid, date: alarm.date, ringtoneName: alarm.mediaLabel, repeatWeekdays: alarm.repeatWeekdays, snoonzeEnabled: alarm.snoozeEnabled)
                } else {
                    scheduler.cancelNotification(ByUUIDStr: uuid)
                }
...
//            UNUserNotificationCenter.current().removeAllPendingNotificationRequests()
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        guard
            ...
            let addEditController = dist.topViewController as? AddAlarmVC
        else {return}
