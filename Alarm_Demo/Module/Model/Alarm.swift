...
class Alarm: Codable {
    ...
    convenience init() {
        self.init(uuid: UUID(), date: Date(), enabled: true, snoozeEnabled: false, repeatWeekdays: [], mediaID: "", mediaLabel: "bell", label: "Alarm", snoozeCount: 0)
    }
    
    init(uuid: UUID, date: Date, enabled: Bool, snoozeEnabled: Bool, repeatWeekdays: [Int], mediaID: String, mediaLabel: String, label: String, snoozeCount: Int) {
        self.uuid = uuid
        self.date = date
    ...
    required init(from decoder: Decoder) throws {
        ...
        self.snoozeCount = try container.decode(Int.self, forKey: Alarm.CodingKeys.snoozeCount)
    }
    ...
    func encode(to encoder: Encoder) throws {
        ...
        try container.encode(self.snoozeCount, forKey: Alarm.CodingKeys.snoozeCount)
    }
}
