...
class NotificationScheduler : NotificationSchedulerDelegate
{
    ...
    func registerNotificationCategories() {
        ...
        let snoozeAction = UNNotificationAction(identifier: Identifier.snoozeActionIdentifier, title: "Snooze2", options: [.foreground])
    ...
    private func getNotificationDates(baseDate date: Date, onWeekdaysForNotify weekdays:[Int]) -> [Date]
    {
        ...
        print("notificationDates:-", notificationDates,
              "calendar:-", calendar,
              "now:-", now,
              "flags:-", flags,
        ...
        if weekdays.isEmpty {
            ...
            for wdIndex in weekdays {
                ...
                if compare(weekday: wd, with: weekday) == .same {
    ...
    func setNotification(alarmLabel: String ,date: Date, ringtoneName: String, repeatWeekdays: [Int], snoozeEnabled: Bool, onSnooze: Bool, uuid: String) {
        ...
        for d in datesForNotification {
            ...
            notificationContent.title = "AlarmBack"
            notificationContent.body = alarmLabel
            ...
            notificationContent.userInfo = ["snooze" : snoozeEnabled, "uuid": uuid, "soundName": ringtoneName, "alarmLabel": alarmLabel]
    ...
    func setNotificationForSnooze(alarmLabel: String ,ringtoneName: String, snoozeMinute: Int, uuid: String, dateTime: Date) {
        let calendar = Calendar(identifier: Calendar.Identifier.gregorian)
        let now = Date()
        let snoozeDate = (calendar as NSCalendar).date(byAdding: NSCalendar.Unit.minute, value: snoozeMinute, to: dateTime, options:.matchStrictly)!
        setNotification(alarmLabel: alarmLabel ,date: snoozeDate, ringtoneName: ringtoneName, repeatWeekdays: [], snoozeEnabled: true, onSnooze: true, uuid: uuid)
    }
    ...
    func updateNotification(alarmLabel: String,ByUUIDStr uuid: String, date: Date, ringtoneName: String, repeatWeekdays: [Int], snoonzeEnabled: Bool) {
        ...
        setNotification(alarmLabel: alarmLabel ,date: date, ringtoneName: ringtoneName, repeatWeekdays: repeatWeekdays, snoozeEnabled: snoonzeEnabled, onSnooze: false, uuid: uuid)
    }
